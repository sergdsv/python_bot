...
class ManageGraph:
    ...
    def __init__(self, g_name, g_data_source, rrd_data=None, g_dir=current_dir, g_start='1d'):
        self.g_dir = g_dir
        self.g_name = g_name
    ...
    def update(self):
        if os.path.exists(self.rrd_file):
            str_data = ''
...
graph = ManageGraph(g_name='temp',
                    g_start="1d",
                    rrd_data=[dht_device.temperature, dht_device.humidity],
                    g_data_source=[
                            {
                                'name': 'ds1',
                                'color': '48C4EC',
